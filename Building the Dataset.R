#### MASTER'S THESIS - EFFECT OF CLIMATE CHANGE ON PEACE (STABILITY) ####
...
WDI <- WDI %>% mutate(year = `ï..Time`, shdynmort_wb =`Mortality.rate..under.5..per.1.000.live.births...SH.DYN.MORT.`)%>% 
  subset(year>=1990)%>%
...
WDI2 <- WDI2 %>% mutate(year = `ï..Time`, ODAus18 = `Net.official.development.assistance.received..constant.2018.US....DT.ODA.ODAT.KD.`,
...
OECD <- OECD %>% subset(Ownership == "All undertakings (=1+3)" & DBRA == "TOT" & ITYP == "TOT") %>% 
  mutate(GIC_mil = `Value`, ISO = `ï..COU`) %>% 
...
write.csv(Data,"C:/Users/..../data.csv", row.names = TRUE)
